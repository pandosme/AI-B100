...
bool Video_Start_YUV(unsigned int width, unsigned int height, const char* image_fit) {
	yuvProvider = createImgProvider(width, height, 2, VDO_FORMAT_YUV, image_fit);
    if (!yuvProvider) {
...
bool Video_Start_RGB(unsigned int width, unsigned int height) {
	rgbProvider = createImgProvider(width, height, 1, VDO_FORMAT_JPEG, "crop");
    if (!rgbProvider) {
        LOG_WARN("%s: Could not create image provider\n", __func__);
		return false;
	}
