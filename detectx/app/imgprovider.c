 ...
 */
static bool createStream(ImgProvider_t* provider,
                         unsigned int w,
                         unsigned int h,
                         const char* imageFit);
...
ImgProvider_t* createImgProvider(unsigned int w,
                                unsigned int h,
    ...
    if (!createStream(provider, w, h, imageFit)) {
...
void destroyImgProvider(ImgProvider_t* provider) {
    ...
    if (provider->vdoStream) {
        g_object_unref(provider->vdoStream);
        provider->vdoStream = NULL;
    }
...
bool createStream(ImgProvider_t* provider,
                  unsigned int w,
                  unsigned int h,
                  const char* imageFit) {
    ...
    vdo_map_set_string(vdoMap, "image.fit", imageFit ? imageFit : "crop");
...
VdoBuffer* getLastFrameBlocking(ImgProvider_t* provider) {
    ...
    while (g_queue_get_length(provider->deliveredFrames) < 1) {
        ...
        deadline.tv_sec += 1;
        int waitResult = pthread_cond_timedwait(&provider->frameDeliverCond, &provider->frameMutex, &deadline);
        if (waitResult == ETIMEDOUT) {
            goto errorExit;
        }
        if (waitResult) {
            syslog(LOG_ERR, "%s: Failed to wait on condition: %s", __func__, strerror(waitResult));
            goto errorExit;
        }
    }
...
bool stopFrameFetch(ImgProvider_t* provider) {
    ...
    vdo_stream_stop(provider->vdoStream);
    pthread_mutex_lock(&provider->frameMutex);
    pthread_cond_broadcast(&provider->frameDeliverCond);
    pthread_mutex_unlock(&provider->frameMutex);
